...
def train_engine(
    ...
    try:
        for epoch in range(train_states["start_epoch"], config["EPOCHS"]):
            ...
            # --- Log train/log.txt ONCE per epoch (artifact) ---
            if is_main_process():
                log_txt_path = os.path.join(logger.logdir, "log.txt")
                if os.path.exists(log_txt_path):
                    mlflow.log_artifact(log_txt_path, artifact_path="logs")

            # Save checkpoint:
            if (epoch + 1) % config["SAVE_CHECKPOINT_PER_EPOCH"] == 0:
                ckpt_path = os.path.join(outputs_dir, f"checkpoint_{epoch}.pth")
                save_checkpoint(
                    model=model,
                    path=ckpt_path,
                    states=train_states,
                ...
                # Log checkpoint as an MLflow artifact (main process only)
                if is_main_process() and os.path.exists(ckpt_path):
